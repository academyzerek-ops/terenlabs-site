...
## Файлы
- `products.json` — все продукты (курсы, тесты, кейсы, обзоры, разборы брендов): метаданные карточки.
- `brands.json` — разборы бизнес-моделей брендов (тип `bm`): карточка + готовое тело HTML для нативного рендера на `/brands/<slug>`.
- `literature.json` — редакционные рекомендации книг: библиография, краткое описание, темы и связи с главой и разбором. Тексты книг здесь не хранятся.
...
| Поле | Обяз. | Пример |
|------|-------|--------|
| type | да | "test" / "course" / "case" / "review" / "bm" (finmodel снят с прода 09.09.2026) |
...
| interactive | нет | true для кейса/обзора с собственной механикой |
...
## Добавить книгу в «Литературу»
1. Добавить карточку в `literature.json`: название, автора, год, краткое описание и три причины читать.
2. Привязать книгу к главе в `lib/literature.ts` через ключ `<slug-трека>:<файл-главы>`.
3. Указать связанные главу и разбор. Магазинные ссылки, цена и текст книги не добавляются: раздел только рекомендует чтение по теме.

## Два регистра контента
- **Регистр А — Казахстан, МСБ**: обзоры ниш, кейсы, тенге, местные налоги.
