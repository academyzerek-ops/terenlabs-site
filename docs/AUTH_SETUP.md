...
провайдеры telegram / email / google).

0. **Telegram**: официальный Login Widget (`components/TelegramLogin.tsx`) → бэкенд
   `/api/ocean/auth/telegram`. Бот нужен только для входа (Mini App удалён 09.09.2026);
   домен сайта прописан в @BotFather → /setdomain. Старые Telegram-аккаунты находятся по tg_id.
...
`/auth/onboarding` — имя, год рождения, пол, страна (KZ, RU, KG, UZ, BY; область убрана 09.09.2026). Имя вводится руками,
