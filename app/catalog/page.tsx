...
const SECTION: Record<ProductType | "all", { eyebrow: string; title: string; desc: string }> = {
  ...
  test: {
    ...
    desc: "Один путь для всех. Уровень открывается, когда сдан предыдущий; балл считает сервер, открытые кейсы проверяет ИИ-акулёнок.",
  },
  case: {
    eyebrow: "Кейсы · Предприниматель",
    title: "Чужой опыт как учитель",
    desc: "Разбираем реальные ситуации своего дела: где теряют деньги и как этого не допустить.",
  },
  review: {
    eyebrow: "Предприниматель · Ниши",
    title: "Обзоры ниш",
...
export default async function CatalogPage({
  ...
  return (
    <>
      ...
      <Container className={`py-10 ${t === "test" ? "!max-w-[1180px]" : ""}`}>
        {t === "test" ? (
          <TestPath />
        ) : list.length > 0 ? (
