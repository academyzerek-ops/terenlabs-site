...
const I = {
  ...
  brick: <path d="M2.5 6.5 8 4l5.5 2.5L8 9zM2.5 6.5v3L8 12l5.5-2.5v-3M8 9v3" />,
  coin: <path d="M8 2.5v11M10.2 4.6H7a1.6 1.6 0 0 0 0 3.2h2a1.6 1.6 0 0 1 0 3.2H5.6" />,
...
// Панель по типу занятия, не по аудитории (решение Адиля 06.09): Академия и Тесты одни
// на всех, разборы трёх видов. Это решение в силе — группы ниже устроены по занятию.
//
// Дополнено 11.09.2026 (разворот к четырём хабам, канон 16_HUBS.md): хабов стало четыре,
// и жить только дверями на главной они больше не могут — войдя в хаб, человек в первом же
// клике терял ветку. Хабы вынесены отдельной группой наверх, разбивка остального
// по типу занятия сохранена как была.
const GROUPS: { title: string; items: Item[] }[] = [
  {
    title: "Хабы",
    items: [
      { label: "Фундамент", href: "/money", icon: I.brick, match: (p) => p === "/money" },
      { label: "Предприниматель", href: "/delo", icon: I.shop, match: (p) => p.startsWith("/delo") },
      { label: "Фаундер", href: "/startup", icon: I.rocket, match: (p) => p === "/startup" },
      { label: "Инвестор", href: "/invest", icon: I.coin, match: (p) => p === "/invest" },
    ],
  },
  {
    title: "Учиться",
