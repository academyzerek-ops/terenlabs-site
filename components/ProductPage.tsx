...
const FAQ_FINMODEL = [
  ...
  { q: "Это правда бесплатно?", a: "Да. Сборка и скачивание бесплатны — нужен только вход на сайте." },
...
const FAQ_INDFM = [
  ...
  { q: "Как заказать?", a: "Оставляешь заявку на сайте — эксперт связывается по указанному контакту и уточняет детали." },
];
...
const FORMAT_BY_TYPE: Record<string, string[]> = {
  finmodel: [
    "Бесплатно — сборка на сайте готовится",
...
export function ProductPage({ p }: { p: Product }) {
  ...
  const cta =
    p.type === "test"
      ...
      : isPaidFinmodel
      ? // независимая заявка на сайте (форма ниже, #zayavka)
      ...
      : // бесплатные финпродукты: сборка на сайте готовится (анкеты жили в Mini App,
        // удалённом 09.09.2026) — пока ведём на заявку
        { href: "#zayavka", label: "Оставить заявку" };
            ...
            <aside className="rounded-[8px] border border-line bg-subtle p-5 lg:sticky lg:top-20">
              ...
              {!p.stub && (
                <p className="mt-3 text-[12px] leading-relaxed text-faint">
                  {isPaidFinmodel
                    ? "Заявка прямо на сайте — ответим по вашему контакту"
                    : "Сборка на сайте готовится — пока соберём по заявке"}
                </p>
              )}
            </aside>
          </div>
